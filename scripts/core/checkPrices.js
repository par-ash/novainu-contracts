...
async function checkPrices(vaultPriceFeed1, vaultPriceFeed2, tokenArr) {
  for (let i = 0; i < tokenArr.length; i++) {
    ...
    console.log("maxPrice1: {}", maxPrice1)
    const minPrice1 = await vaultPriceFeed1.getPrice(token.address, false, true, true)
    console.log("minPrice1: {}", minPrice1)
    const diff1 = maxPrice1.sub(minPrice1)
    const spread1 = diff1.mul(1000000).div(minPrice1)

    let maxPrice2, minPrice2, diff2, spread2
    if (vaultPriceFeed2) {
      maxPrice2 = await vaultPriceFeed2.getPrice(token.address, true, true, true, {gasLimit:3e11})
      console.log("maxPrice2: {}", maxPrice2)
      minPrice2 = await vaultPriceFeed2.getPrice(token.address, false, true, true, {gasLimit:3e11})
...
async function checkPricesArb() {
  const vaultPriceFeed1 = await contractAt("VaultPriceFeed", "0x8A87239a237c14e385b3D7289995AC48931F8a93")
  // let vaultPriceFeed2
  const vaultPriceFeed2 = await contractAt("VaultPriceFeed", "0x8A87239a237c14e385b3D7289995AC48931F8a93")

  const { btc, eth, usdc, link, uni, usdt, mim, frax, dai } = tokens
  const tokenArr = [btc, eth, usdc, usdt]//[btc, eth, usdc, link, uni, usdt, mim, frax, dai]
