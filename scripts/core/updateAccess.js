...
async function main() {
  const stakedNovaTracker = await contractAt("RewardTracker", "0x2bD10f8E93B3669b6d42E74eEedC65dd1B0a1342")
  const bonusNovaTracker = await contractAt("RewardTracker", "0x908C4D94D34924765f1eDc22A1DD098397c59dD4")
  const feeNovaTracker = await contractAt("RewardTracker", "0x4d268a7d4C16ceB5a606c173Bd974984343fea13")
  ...
  await printRewardTracker(stakedNovaTracker, "stakedNovaTracker")
  await printRewardTracker(bonusNovaTracker, "bonusNovaTracker")
  await printRewardTracker(feeNovaTracker, "feeNovaTracker")

  await printRewardTracker(stakedNlpTracker, "stakedNlpTracker")
  await printRewardTracker(feeNlpTracker, "feeNlpTracker")

  const nlp = await contractAt("MintableBaseToken", "0x01234181085565ed162a948b6a5e88758CD7c7b8")
  const usdg = await contractAt("USDG", "0xc0253c3cC6aa5Ab407b5795a04c28fB063273894")
  // const nova = await contractAt("MintableBaseToken", "0x62edc0692BD897D2295872a9FFCac5425011c661")
  // const esNova = await contractAt("MintableBaseToken", "0xFf1489227BbAAC61a9209A08929E4c2a526DdD17")
  const bnNova = await contractAt("MintableBaseToken", "0x8087a341D32D445d9aC8aCc9c14F5781E04A26d2")

  await printToken(nlp, "nlp")
  await printUsdg(usdg, "usdg")
  // await printToken(nova, "nova")
  // await printToken(esNova, "esNova")
  await printToken(bnNova, "bnNova")
  ...
  // await signalGov(prevGov, nlp, nextGov, "nlp")
  // await signalGov(prevGov, nova, nextGov, "nova")
  // await signalGov(prevGov, esNova, nextGov, "esNova")
  // await signalGov(prevGov, bnNova, nextGov, "bnNova")

  await updateToken(nova, "nova")
  await updateToken(esNova, "esNova")
  await updateToken(bnNova, "bnNova")

  await updateHandler(stakedNovaTracker, "stakedNovaTracker")
  await updateHandler(bonusNovaTracker, "bonusNovaTracker")
  await updateHandler(feeNovaTracker, "feeNovaTracker")
  await updateHandler(stakedNlpTracker, "stakedNlpTracker")
  await updateHandler(feeNlpTracker, "feeNlpTracker")

  await updateRewardTrackerGov(stakedNovaTracker, "stakedNovaTracker")

  await updateRewardTrackerGov(bonusNovaTracker, "bonusNovaTracker")
  await updateRewardTrackerGov(feeNovaTracker, "feeNovaTracker")
  await updateRewardTrackerGov(stakedNlpTracker, "stakedNlpTracker")
  await updateRewardTrackerGov(feeNlpTracker, "feeNlpTracker")

  await updateGov(nlp, "nlp")
  await updateGov(usdg, "usdg")
  // await updateGov(nova, "nova")
  // await updateGov(esNova, "esNova")
  await updateGov(bnNova, "bnNova")
  ...
  const nlpManager = await contractAt("NlpManager", "0xe1ae4d4b06A5Fe1fc288f6B4CD72f9F8323B107F")
  ...
  await updateGov(nlpManager, "nlpManager")
