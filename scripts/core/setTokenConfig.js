...
async function getArbValues() {
  ...
  const tokenArr = [ btc, eth, usdc, usdt] //[ btc, eth, usdc, link, uni, usdt, frax, dai ]
  console.log(eth.address);
  const vaultTokenInfo = await reader.getVaultTokenInfoV2(vault.address, eth.address, 1, tokenArr.map(t => t.address), {gasLimit:1000000000000000})
