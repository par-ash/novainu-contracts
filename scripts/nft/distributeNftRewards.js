...
const distribute = async () => {
  ...
  const stakedNovaBalances = await processFile(inputDir + "staked-nova-balances.csv")
  const vestedBalances = await processFile(inputDir + "vested-balances.csv")

  const holders = {}

  for (let i = 0; i < stakedNovaBalances.length; i++) {
    const tokenHolder = stakedNovaBalances[i]
  ...
  if (shouldSendTokens) {
    await sendTxn(esNova.approve(batchSender.address, expandDecimals(totalEsNova, 18)), "esNova.approve")
  }
  ...
  for (let i = 0; i < balanceList.length; i++) {
    ...
    const esNovaValue = (totalEsNova - 1) * balance / totalBalance
    const esNovaAmount = ethers.utils.parseUnits(esNovaValue.toFixed(4), 18)

    accounts.push(account)
    amounts.push(esNovaAmount)
    totalEsNovaAmount = totalEsNovaAmount.add(esNovaAmount)

    console.log(`${i+1},${account},${esNovaValue},${esNovaAmount.toString()}`)

    if (accounts.length === batchSize && shouldSendTokens) {
      console.log("sending batch", i, accounts.length, amounts.length)
      await sendTxn(batchSender.send(esNova.address,  accounts, amounts), "batchSender.send")
  ...
  console.log("totalEsNovaAmount", totalEsNovaAmount.toString())
}
