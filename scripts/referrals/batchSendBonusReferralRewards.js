...
async function getArbValues() {
  ...
  const nova = await contractAt("Token", "0xfc5A1A6EB076a2C7aD06eD22C90d7E710E35ad0a")
  const data = arbitrumData
  const gasLimit = 30000000
  const totalNova = 766
  const totalUsd = 44821

  return { batchSender, nova, data, gasLimit, totalNova, totalUsd }
}

async function getAvaxValues() {
  const batchSender = await contractAt("BatchSender", "0xF0f929162751DD723fBa5b86A9B3C88Dc1D4957b")
  const nova = await contractAt("Token", "0x62edc0692BD897D2295872a9FFCac5425011c661")
  const data = avaxData
...
async function main() {
  ...
  const { batchSender, nova, data, totalNova, totalUsd, gasLimit } = await getValues()
  ...
  for (let i = 0; i < affiliatesData.length; i++) {
    ...
    const amount = bigNumberify(rebateUsd).mul(expandDecimals(totalNova, 18)).div(expandDecimals(totalUsd, 30))
  ...
  if (shouldSendTxn) {
    ...
    await sendTxn(nova.approve(batchSender.address, totalAmount, { gasLimit: 1000000 }), "nova.approve")
    ...
    await processBatch([affiliateAccounts, affiliateAmounts], batchSize, async (currentBatch) => {
      ...
      await sendTxn(batchSender.sendAndEmit(nova.address, accounts, amounts, affiliateRewardsTypeId, { gasLimit }), "batchSender.sendAndEmit(nova, affiliate rewards)")
    })
  }
}
