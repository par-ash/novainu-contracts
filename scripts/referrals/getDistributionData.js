...
const ARBITRUM_SUBGRAPH_ENDPOINT = 'https://api.thegraph.com/subgraphs/name/nova-io/nova-arbitrum-referrals'
const AVALANCHE_SUBGRAPH_ENDPOINT = 'https://api.thegraph.com/subgraphs/name/nova-io/nova-avalanche-referrals'

const BigNumber = ethers.BigNumber
const { formatUnits, parseUnits } = ethers.utils
const SHARE_DIVISOR = BigNumber.from("1000000000") // 1e9
const BONUS_TIER = 2 // for EsNOVA distributions
...
async function queryDistributionData(network, fromTimestamp, toTimestamp, account, novaPrice, esnovaRewards) {
  if (novaPrice) {
    novaPrice = parseUnits(novaPrice, USD_DECIMALS)
  }
  if (esnovaRewards) {
    esnovaRewards = parseUnits(esnovaRewards, NOVA_DECIMALS)
  }
  ...
  if (novaPrice && esnovaRewards) {
    const esnovaRewardsUsdLimit = esnovaRewards.mul(novaPrice).div(expandDecimals(1, NOVA_DECIMALS))
    let esnovaRewardsUsdTotal = BigNumber.from(0)
    Object.values(referrersRebatesData).forEach(data => {
      if (data.tierId !== BONUS_TIER) {
        return
      }
      data.esnovaRewardsUsd = data.volume.div(1000).div(20) // 0.1% margin fee, 0.05% of fee is EsNOVA bonus rewards
      data.esnovaRewards = data.esnovaRewardsUsd
        .mul(expandDecimals(1, USD_DECIMALS))
        .div(novaPrice)
        .div(expandDecimals(1, 12))
      esnovaRewardsUsdTotal = esnovaRewardsUsdTotal.add(data.esnovaRewardsUsd)
    })

    if (esnovaRewardsUsdTotal.gt(esnovaRewardsUsdLimit)) {
      const denominator = esnovaRewardsUsdTotal.mul(USD_DECIMALS).div(esnovaRewardsUsdLimit)
      Object.values(referrersRebatesData).forEach(data => {
        data.esnovaRewardsUsd = data.esnovaRewardsUsd.mul(USD_DECIMALS).div(denominator)
        data.esnovaRewards = data.esnovaRewardsUsd
          .mul(expandDecimals(1, USD_DECIMALS))
          .div(novaPrice)
          .div(expandDecimals(1, 12))
  ...
  for (const data of Object.values(referrersRebatesData)) {
    ...
    consoleData.push({
      ...
      tierId: data.tierId,
      "esnovaRewards, $": data.esnovaRewardsUsd ? formatUnits(data.esnovaRewardsUsd, USD_DECIMALS) : null,
      esnovaRewards: data.esnovaRewards ? formatUnits(data.esnovaRewards, NOVA_DECIMALS) : null,
    })
    output.referrers.push({
      ...
      tierId: data.tierId,
      esnovaRewards: data.esnovaRewards ? data.esnovaRewards.toString() : null,
      esnovaRewardsUsd: data.esnovaRewardsUsd ? data.esnovaRewardsUsd.toString() : null,
    })
  }
...
async function main() {
  ...
  parser.add_argument('-g', '--nova-price', { help: 'NOVA TWAP price' })
  parser.add_argument('-e', '--esnova-rewards', {
    help: 'Amount of EsNOVA to distribute to Tier 3',
    default: "5000"
  })
