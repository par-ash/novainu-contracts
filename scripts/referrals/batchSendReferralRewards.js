...
async function getArbValues() {
  ...
  const esNova = await contractAt("Token", "0xf42Ae1D54fd613C9bb14810b0588FaAa09a426cA")
  const nativeTokenPrice = ethPrice
  const data = arbitrumData

  return { batchSender, esNova, nativeTokenPrice, data }
}

async function getAvaxValues() {
  const batchSender = await contractAt("BatchSender", "0xF0f929162751DD723fBa5b86A9B3C88Dc1D4957b")
  const esNova = await contractAt("Token", "0xFf1489227BbAAC61a9209A08929E4c2a526DdD17")
...
async function main() {
  ...
  const { batchSender, esNova, nativeTokenPrice, data } = await getValues()
  ...
  let totalEsNovaAmount = bigNumberify(0)
  const affiliateAccounts = []
  const affiliateAmounts = []
  const discountAccounts = []
  const discountAmounts = []
  const esNovaAccounts = []
  const esNovaAmounts = []

  for (let i = 0; i < affiliatesData.length; i++) {
    const { account, rebateUsd, esnovaRewardsUsd } = affiliatesData[i]
    ...
    if (esnovaRewardsUsd) {
      const esNovaAmount = bigNumberify(esnovaRewardsUsd).mul(expandDecimals(1, 18)).div(expandDecimals(novaPrice, 30))
      esNovaAccounts.push(account)
      esNovaAmounts.push(esNovaAmount)
      totalEsNovaAmount = totalEsNovaAmount.add(esNovaAmount)
    }
  }
  ...
  console.log(`total esNova`, ethers.utils.formatUnits(totalEsNovaAmount, 18))
  ...
  if (shouldSendTxn) {
    ...
    await sendTxn(esNova.approve(batchSender.address, totalEsNovaAmount), "esNova.approve")

    await processBatch([esNovaAccounts, esNovaAmounts], batchSize, async (currentBatch) => {
      printBatch(currentBatch)

      const accounts = currentBatch.map((item) => item[0])
      const amounts = currentBatch.map((item) => item[1])

      await sendTxn(batchSender.sendAndEmit(esNova.address, accounts, amounts, affiliateRewardsTypeId), "batchSender.sendAndEmit(nativeToken, esNova affiliate rewards)")
    })
  }
}
