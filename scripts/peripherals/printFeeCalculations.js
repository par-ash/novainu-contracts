...
async function getNovaPrice(ethPrice) {
...
async function getArbValues() {
  ...
  const stakedNova = await contractAt("Token", "0xd2D1162512F927a7e282Ef43a362659E4F2a728F", signer)
  const stakedNovaSupply = await stakedNova.totalSupply()
  const { totalTransferAmount: keeperCosts } = await getArbKeeperValues()
  const nlpManager = await contractAt("NlpManager", "0x321F653eED006AD1C29D174e17d96351BDe22649", signer)
  const nlpAum = await nlpManager.getAum(true)

  return { vault, reader, tokens, tokenInfo, nativeTokenPrice, feesUsd, stakedNova, stakedNovaSupply, keeperCosts, nlpManager, nlpAum }
}
...
async function getAvaxValues() {
  ...
  const nativeTokenPrice = tokenInfo[tokens.nativeToken.address].maxPrice
  const feesUsd = await getFeesUsd(vault, reader, tokenInfo)
  const stakedNova = await contractAt("Token", "0x4d268a7d4C16ceB5a606c173Bd974984343fea13", signer)
  const stakedNovaSupply = await stakedNova.totalSupply()
  const { totalTransferAmount: keeperCosts } = await getAvaxKeeperValues()
  const nlpManager = await contractAt("NlpManager", "0xe1ae4d4b06A5Fe1fc288f6B4CD72f9F8323B107F", signer)
  const nlpAum = await nlpManager.getAum(true)
...
async function main() {
  ...
  const novaPrice = await getNovaPrice(ethPrice)

  const data = [
    ["ETH Price", formatAmount(ethPrice, 30, 2)],
    ["AVAX Price", formatAmount(avaxPrice, 30, 2)],
    ["NOVA Price", formatAmount(novaPrice, 30, 2)],
    ["ARB Fees", formatAmount(values.arbitrum.feesUsd, 30, 2)],
    ["AVAX Fees", formatAmount(values.avax.feesUsd, 30, 2)],
    ["ARB sbfNOVA", formatAmount(values.arbitrum.stakedNovaSupply, 18, 2)],
    ["AVAX sbfNOVA", formatAmount(values.avax.stakedNovaSupply, 18, 2)],
    ...
    ["NLP AUM (ARB)", formatAmount(values.arbitrum.nlpAum, 30, 2)],
    ["NLP AUM (AVAX)", formatAmount(values.avax.nlpAum, 30, 2)],
  ]
