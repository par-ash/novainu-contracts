...
async function main() {
  ...
  const nova = new Token(42161, "0xfc5A1A6EB076a2C7aD06eD22C90d7E710E35ad0a", 18, "SYMBOL", "NAME")
  ...
  console.log("NFT ID,Start (ETH),End (ETH),ETH Liquidity, NOVA Liquidity")
  for (let i = 0; i < nftIds.length; i++) {
    ...
    const start = tickToPrice(nova, weth, positionInfo.tickUpper).toSignificant(6)
    const end = tickToPrice(nova, weth, positionInfo.tickLower).toSignificant(6)
    const ethLiquidity = position.amount0.toSignificant(6)
    const novaLiquidity = position.amount1.toSignificant(6)

    console.log(`${nftId},${start},${end},${ethLiquidity},${novaLiquidity}`)
  }
}
