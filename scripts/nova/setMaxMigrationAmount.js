...
async function main() {
  ...
  const novaMigrator = await contractAt("NovaMigrator", "0x0472F402EA8E301D7595545884Ad4C420E9865d6")
  const gmt = {
    name: "GMT",
  ...
  if (shouldApprove) {
    for (let i = 0; i < tokens.length; i++) {
      ...
      const migratedAmount = await novaMigrator.migratedAmounts(account, token.contract.address)
      ...
      await sendTxn(novaMigrator.setMaxMigrationAmount(account, token.contract.address, totalAmount), message)
    }
  }
}
