...
describe("TokenManager", function () {
  ...
  beforeEach(async () => {
    ...
    novaTimelock = await deployContract("NovaTimelock", [
      wallet.address,
      5 * 24 * 60 * 60,
      7 * 24 * 60 * 60,
      user0.address,
  ...
  it("approve", async () => {
    ...
    await expect(tokenManager.connect(wallet).approve(nova.address, user2.address, expandDecimals(5, 18), 1))
      .to.be.revertedWith("TokenManager: action not signalled")
  ...
  it("signalSetGov", async () => {
    await expect(tokenManager.connect(user0).signalSetGov(timelock.address, nova.address, user1.address))
      .to.be.revertedWith("TokenManager: forbidden")

    await tokenManager.connect(wallet).signalSetGov(timelock.address, nova.address, user1.address)
  })

  it("signSetGov", async () => {
    await expect(tokenManager.connect(user0).signSetGov(timelock.address, nova.address, user1.address, 1))
      .to.be.revertedWith("TokenManager: forbidden")

    await expect(tokenManager.connect(signer2).signSetGov(timelock.address, nova.address, user1.address, 1))
      .to.be.revertedWith("TokenManager: action not signalled")
    ...
    await tokenManager.connect(signer2).signSetGov(timelock.address, nova.address, user1.address, 1)

    await expect(tokenManager.connect(signer2).signSetGov(timelock.address, nova.address, user1.address, 1))
      .to.be.revertedWith("TokenManager: already signed")

    await tokenManager.connect(signer1).signSetGov(timelock.address, nova.address, user1.address, 1)
  })

  it("setGov", async () => {
    await nova.setGov(novaTimelock.address)

    await expect(tokenManager.connect(user0).setGov(novaTimelock.address, nova.address, user1.address, 1))
      .to.be.revertedWith("TokenManager: forbidden")

    await expect(tokenManager.connect(wallet).setGov(novaTimelock.address, nova.address, user1.address, 1))
      .to.be.revertedWith("TokenManager: action not signalled")

    await tokenManager.connect(wallet).signalSetGov(novaTimelock.address, nova.address, user1.address)

    await expect(tokenManager.connect(wallet).setGov(user2.address, nova.address, user1.address, 1))
      .to.be.revertedWith("TokenManager: action not signalled")

    await expect(tokenManager.connect(wallet).setGov(novaTimelock.address, user0.address, user1.address, 1))
      .to.be.revertedWith("TokenManager: action not signalled")

    await expect(tokenManager.connect(wallet).setGov(novaTimelock.address, nova.address, user2.address, 1))
      .to.be.revertedWith("TokenManager: action not signalled")

    await expect(tokenManager.connect(wallet).setGov(novaTimelock.address, nova.address, user1.address, 1 + 1))
      .to.be.revertedWith("TokenManager: action not signalled")

    await expect(tokenManager.connect(wallet).setGov(novaTimelock.address, nova.address, user1.address, 1))
      .to.be.revertedWith("TokenManager: action not authorized")

    await tokenManager.connect(signer0).signSetGov(novaTimelock.address, nova.address, user1.address, 1)

    await expect(tokenManager.connect(wallet).setGov(novaTimelock.address, nova.address, user1.address, 1))
      .to.be.revertedWith("TokenManager: insufficient authorization")

    await expect(novaTimelock.connect(wallet).signalSetGov(nova.address, user1.address))
      .to.be.revertedWith("NovaTimelock: forbidden")

    await tokenManager.connect(signer2).signSetGov(novaTimelock.address, nova.address, user1.address, 1)

    await expect(novaTimelock.connect(wallet).setGov(nova.address, user1.address))
      .to.be.revertedWith("NovaTimelock: action not signalled")

    await tokenManager.connect(wallet).setGov(novaTimelock.address, nova.address, user1.address, 1)
    ...
    expect(await nova.gov()).eq(novaTimelock.address)
    await novaTimelock.connect(wallet).setGov(nova.address, user1.address)
    expect(await nova.gov()).eq(user1.address)
  })
})
