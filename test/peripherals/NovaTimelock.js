...
describe("NovaTimelock", function () {
  ...
  beforeEach(async () => {
    ...
    timelock = await deployContract("NovaTimelock", [
      wallet.address,
      5 * 24 * 60 * 60,
      7 * 24 * 60 * 60,
  ...
  it("inits", async () => {
    ...
    await expect(deployContract("NovaTimelock", [
      wallet.address,
      7 * 24 * 60 * 60 + 1,
      7 * 24 + 60 * 60,
      rewardManager.address,
      ...
      mintReceiver.address,
      1000
    ])).to.be.revertedWith("NovaTimelock: invalid _buffer")
    ...
    await expect(deployContract("NovaTimelock", [
      ...
      mintReceiver.address,
      1000
    ])).to.be.revertedWith("NovaTimelock: invalid _longBuffer")
  })
  ...
  it("setTokenConfig", async () => {
    ...
    await expect(timelock.connect(user0).setTokenConfig(
      ...
      0,
      0
    )).to.be.revertedWith("NovaTimelock: forbidden")
    ...
    await expect(timelock.connect(wallet).setTokenConfig(
      ...
      0,
      0
    )).to.be.revertedWith("NovaTimelock: token not yet whitelisted")
  ...
  it("setBuffer", async () => {
    const timelock0 = await deployContract("NovaTimelock", [
      user1.address,
      3 * 24 * 60 * 60,
      7 * 24 * 60 * 60,
      rewardManager.address,
    ...
    await expect(timelock0.connect(user1).setBuffer(3 * 24 * 60 * 60 - 10))
      .to.be.revertedWith("NovaTimelock: buffer cannot be decreased")
  ...
  it("setMaxLeverage", async () => {
    ...
    await expect(timelock.connect(wallet).setMaxLeverage(vault.address, 49 * 10000))
      .to.be.revertedWith("NovaTimelock: invalid _maxLeverage")
  ...
  it("approve", async () => {
    ...
    await expect(timelock.connect(wallet).approve(dai.address, user1.address, expandDecimals(100, 18)))
      .to.be.revertedWith("NovaTimelock: action not signalled")
    ...
    await expect(timelock.connect(wallet).approve(dai.address, user1.address, expandDecimals(100, 18)))
      .to.be.revertedWith("NovaTimelock: action time not yet passed")
    ...
    await expect(timelock.connect(wallet).cancelAction(action0))
      .to.be.revertedWith("NovaTimelock: invalid _action")
  ...
  it("processMint", async () => {
    ...
    await nova.setGov(timelock.address)

    await expect(timelock.connect(user0).processMint(nova.address, user1.address, expandDecimals(100, 18)))
      .to.be.revertedWith("NovaTimelock: forbidden")

    await expect(timelock.connect(wallet).processMint(nova.address, user1.address, expandDecimals(100, 18)))
      .to.be.revertedWith("NovaTimelock: action not signalled")

    await expect(timelock.connect(user0).signalMint(nova.address, user1.address, expandDecimals(100, 18)))
      .to.be.revertedWith("NovaTimelock: forbidden")

    await timelock.connect(wallet).signalMint(nova.address, user1.address, expandDecimals(100, 18))
    ...
    await expect(timelock.connect(wallet).processMint(nova.address, user2.address, expandDecimals(100, 18)))
      .to.be.revertedWith("NovaTimelock: action not signalled")

    await expect(timelock.connect(wallet).processMint(nova.address, user1.address, expandDecimals(101, 18)))
      .to.be.revertedWith("NovaTimelock: action not signalled")

    expect(await nova.balanceOf(timelock.address)).eq(0)
    expect(await nova.balanceOf(user1.address)).eq(0)

    await timelock.connect(wallet).processMint(nova.address, user1.address, expandDecimals(100, 18))
    ...
    const action1 = ethers.utils.solidityKeccak256(["string", "address", "address", "uint256"], ["mint", nova.address, user1.address, expandDecimals(100, 18)])
  ...
  it("withdrawToken", async () => {
    ...
    await expect(timelock.connect(user0).withdrawToken(nova.address, bnb.address, user0.address, 100))
      .to.be.revertedWith("NovaTimelock: forbidden")

    await expect(timelock.connect(wallet).withdrawToken(nova.address, bnb.address, user0.address, 100))
      .to.be.revertedWith("NovaTimelock: action not signalled")

    await expect(timelock.connect(user0).signalWithdrawToken(nova.address, bnb.address, user0.address, 100))
      .to.be.revertedWith("NovaTimelock: forbidden")

    await timelock.connect(wallet).signalWithdrawToken(nova.address, bnb.address, user0.address, 100)
    ...
    await expect(timelock.connect(wallet).withdrawToken(nova.address, dai.address, user0.address, 100))
      .to.be.revertedWith("NovaTimelock: action not signalled")

    await expect(timelock.connect(wallet).withdrawToken(nova.address, bnb.address, user1.address, 100))
      .to.be.revertedWith("NovaTimelock: action not signalled")

    await expect(timelock.connect(wallet).withdrawToken(nova.address, bnb.address, user0.address, 101))
      .to.be.revertedWith("NovaTimelock: action not signalled")

    await expect(timelock.connect(wallet).withdrawToken(nova.address, bnb.address, user0.address, 100))
      .to.be.revertedWith("ERC20: transfer amount exceeds balance")

    await bnb.mint(nova.address, 100)
    expect(await bnb.balanceOf(user0.address)).eq(0)
    await timelock.connect(wallet).withdrawToken(nova.address, bnb.address, user0.address, 100)
  ...
  it("addExcludedToken", async () => {
    const nova = await deployContract("NOVA", [])
    await expect(timelock.connect(user0).addExcludedToken(nova.address))
      .to.be.revertedWith("NovaTimelock: forbidden")

    expect(await timelock.excludedTokens(nova.address)).eq(false)
    await timelock.connect(wallet).addExcludedToken(nova.address)
    expect(await timelock.excludedTokens(nova.address)).eq(true)
  })
  ...
  it("setInPrivateTransferMode", async () => {
    ...
    await expect(timelock.connect(user0).setInPrivateTransferMode(nova.address, true))
      .to.be.revertedWith("NovaTimelock: forbidden")

    await expect(timelock.connect(wallet).setInPrivateTransferMode(nova.address, true))
      .to.be.revertedWith("BaseToken: forbidden")

    await nova.setGov(timelock.address)

    expect(await nova.inPrivateTransferMode()).eq(false)
    await timelock.connect(wallet).setInPrivateTransferMode(nova.address, true)
    expect(await nova.inPrivateTransferMode()).eq(true)

    await timelock.connect(wallet).setInPrivateTransferMode(nova.address, false)
    expect(await nova.inPrivateTransferMode()).eq(false)

    await timelock.connect(wallet).setInPrivateTransferMode(nova.address, true)
    expect(await nova.inPrivateTransferMode()).eq(true)

    await expect(nova.connect(user0).transfer(user1.address, 100))
      .to.be.revertedWith("BaseToken: msg.sender not whitelisted")

    await timelock.addExcludedToken(nova.address)
    await expect(timelock.connect(wallet).setInPrivateTransferMode(nova.address, true))
      .to.be.revertedWith("NovaTimelock: invalid _inPrivateTransferMode")

    await timelock.connect(wallet).setInPrivateTransferMode(nova.address, false)
    expect(await nova.inPrivateTransferMode()).eq(false)

    await nova.connect(user0).transfer(user1.address, 100)
  })
  ...
  it("setExternalAdmin", async () => {
    ...
    await expect(timelock.connect(wallet).setExternalAdmin(timelock.address, user3.address))
      .to.be.revertedWith("NovaTimelock: invalid _target")
  })
