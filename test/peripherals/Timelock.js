...
describe("Timelock", function () {
  ...
  it("initNlpManager", async () => {
    await expect(timelock.connect(user0).initNlpManager())
      .to.be.revertedWith("Timelock: forbidden")

      await nlp.setGov(timelock.address)
      await usdg.setGov(timelock.address)

      expect(await nlp.isMinter(nlpManager.address)).eq(false)
      expect(await usdg.vaults(nlpManager.address)).eq(false)
      expect(await vault.isManager(nlpManager.address)).eq(false)

      await timelock.initNlpManager()

      expect(await nlp.isMinter(nlpManager.address)).eq(true)
      expect(await usdg.vaults(nlpManager.address)).eq(true)
      expect(await vault.isManager(nlpManager.address)).eq(true)
  })
  ...
  it("initRewardRouter", async () => {
    await expect(timelock.connect(user0).initRewardRouter())
      ...
      expect(await stakedNlpTracker.isHandler(rewardRouter.address)).eq(false)
      expect(await feeNlpTracker.isHandler(rewardRouter.address)).eq(false)
      expect(await nlpManager.isHandler(rewardRouter.address)).eq(false)

      await timelock.initRewardRouter()

      expect(await stakedNlpTracker.isHandler(rewardRouter.address)).eq(true)
      expect(await feeNlpTracker.isHandler(rewardRouter.address)).eq(true)
      expect(await nlpManager.isHandler(rewardRouter.address)).eq(true)
  })
