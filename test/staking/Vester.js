...
describe("Vester", function () {
  ...
  it("inits", async () => {
    ...
    expect(await vester.name()).eq("Vested NOVA")
    expect(await vester.symbol()).eq("veNOVA")
    expect(await vester.vestingDuration()).eq(secondsPerYear)
    expect(await vester.esToken()).eq(esNova.address)
    expect(await vester.pairToken()).eq(AddressZero)
    expect(await vester.claimableToken()).eq(nova.address)
  ...
  it("deposit, claim, withdraw", async () => {
    ...
    await esNova.setMinter(vester.address, true)
    ...
    await esNova.connect(user0).approve(vester.address, expandDecimals(1000, 18))
    ...
    await esNova.mint(user0.address, expandDecimals(1000, 18))
    ...
    expect(await esNova.balanceOf(user0.address)).eq(0)
    ...
    await nova.mint(vester.address, expandDecimals(2000, 18))

    await vester.connect(user0).claim()
    blockTime = await getBlockTime(provider)

    expect(await esNova.balanceOf(user0.address)).eq(0)
    expect(await nova.balanceOf(user0.address)).gt("2730000000000000000")
    expect(await nova.balanceOf(user0.address)).lt("2750000000000000000")

    let novaAmount = await nova.balanceOf(user0.address)
    expect(await vester.balanceOf(user0.address)).eq(expandDecimals(1000, 18).sub(novaAmount))

    expect(await vester.getTotalVested(user0.address)).eq(expandDecimals(1000, 18))
    expect(await vester.cumulativeClaimAmounts(user0.address)).eq(novaAmount)
    expect(await vester.claimedAmounts(user0.address)).eq(novaAmount)
    expect(await vester.claimable(user0.address)).eq(0)
    expect(await vester.pairAmounts(user0.address)).eq(0)
    expect(await vester.lastVestingTimes(user0.address)).eq(blockTime)
    ...
    expect(await esNova.balanceOf(user0.address)).eq(0)
    expect(await nova.balanceOf(user0.address)).gt(expandDecimals(503, 18))
    expect(await nova.balanceOf(user0.address)).lt(expandDecimals(505, 18))

    novaAmount = await nova.balanceOf(user0.address)
    expect(await vester.balanceOf(user0.address)).eq(expandDecimals(1000, 18).sub(novaAmount))
    ...
    await esNova.mint(user0.address, expandDecimals(500, 18))
    await esNova.connect(user0).approve(vester.address, expandDecimals(500, 18))
    ...
    expect(await esNova.balanceOf(user0.address)).eq(0)
    expect(await nova.balanceOf(user0.address)).eq(novaAmount)

    await vester.connect(user0).withdraw()

    expect(await esNova.balanceOf(user0.address)).gt(expandDecimals(989, 18))
    expect(await esNova.balanceOf(user0.address)).lt(expandDecimals(990, 18))
    expect(await nova.balanceOf(user0.address)).gt(expandDecimals(510, 18))
    expect(await nova.balanceOf(user0.address)).lt(expandDecimals(512, 18))
  ...
  it("depositForAccount, claimForAccount", async () => {
    ...
    expect(await esNova.balanceOf(user4.address)).eq(0)
    expect(await nova.balanceOf(user4.address)).gt("2730000000000000000")
    expect(await nova.balanceOf(user4.address)).lt("2750000000000000000")
  ...
  it("handles multiple deposits", async () => {
    ...
    expect(await esNova.balanceOf(user0.address)).gt(expandDecimals(1494, 18))
    expect(await esNova.balanceOf(user0.address)).lt(expandDecimals(1496, 18))
    expect(await nova.balanceOf(user0.address)).gt("5470000000000000000")
    expect(await nova.balanceOf(user0.address)).lt("5490000000000000000")
    expect(await vester.balanceOf(user0.address)).eq(0)
    expect(await vester.getTotalVested(user0.address)).eq(0)
  ...
  it("handles pairing", async () => {
    ...
    await esNova.setMinter(wallet.address, true)
    await esNova.mint(stakedNovaDistributor.address, expandDecimals(50000 * 12, 18))
    ...
    expect(await vester.vestingDuration()).eq(secondsPerYear)
    expect(await vester.esToken()).eq(esNova.address)
    expect(await vester.pairToken()).eq(feeNovaTracker.address)
    expect(await vester.claimableToken()).eq(nova.address)
    expect(await vester.rewardTracker()).eq(stakedNovaTracker.address)
    ...
    // allow vester to transfer feeNovaTracker tokens
    await feeNovaTracker.setHandler(vester.address, true)
    // allow vester to transfer esNova tokens
    await esNova.setHandler(vester.address, true)

    await nova.mint(vester.address, expandDecimals(2000, 18))

    await nova.mint(user0.address, expandDecimals(1000, 18))
    await nova.mint(user1.address, expandDecimals(500, 18))
    await nova.connect(user0).approve(stakedNovaTracker.address, expandDecimals(1000, 18))
    await nova.connect(user1).approve(stakedNovaTracker.address, expandDecimals(500, 18))

    await rewardRouter.connect(user0).stakeNova(expandDecimals(1000, 18))
    await rewardRouter.connect(user1).stakeNova(expandDecimals(500, 18))

    await increaseTime(provider, 24 * 60 * 60)
    await mineBlock(provider)

    expect(await stakedNovaTracker.claimable(user0.address)).gt(expandDecimals(1190, 18))
    expect(await stakedNovaTracker.claimable(user0.address)).lt(expandDecimals(1191, 18))
    expect(await stakedNovaTracker.claimable(user1.address)).gt(expandDecimals(594, 18))
    ...
    expect(await esNova.balanceOf(user3.address)).eq(0)

    await stakedNovaTracker.connect(user0).claim(user2.address)
    await stakedNovaTracker.connect(user1).claim(user3.address)

    expect(await esNova.balanceOf(user0.address)).eq(0)
    expect(await esNova.balanceOf(user1.address)).eq(0)
    expect(await esNova.balanceOf(user2.address)).gt(expandDecimals(1190, 18))
    expect(await esNova.balanceOf(user2.address)).lt(expandDecimals(1191, 18))
    expect(await esNova.balanceOf(user3.address)).gt(expandDecimals(594, 18))
    expect(await esNova.balanceOf(user3.address)).lt(expandDecimals(596, 18))
    ...
    await esNova.mint(user0.address, expandDecimals(2385, 18))
    ...
    await nova.mint(user0.address, expandDecimals(500, 18))
    await nova.connect(user0).approve(stakedNovaTracker.address, expandDecimals(500, 18))
    await rewardRouter.connect(user0).stakeNova(expandDecimals(500, 18))

    await expect(vester.connect(user0).deposit(expandDecimals(2385, 18)))
      .to.be.revertedWith("Vester: max vestable amount exceeded")

    await nova.mint(user2.address, expandDecimals(1, 18))
    await expect(vester.connect(user2).deposit(expandDecimals(1, 18)))
      .to.be.revertedWith("Vester: max vestable amount exceeded")

    expect(await esNova.balanceOf(user0.address)).eq(expandDecimals(2385, 18))
    expect(await esNova.balanceOf(vester.address)).eq(0)
    expect(await feeNovaTracker.balanceOf(user0.address)).eq(expandDecimals(1500, 18))
    expect(await feeNovaTracker.balanceOf(vester.address)).eq(0)

    await vester.connect(user0).deposit(expandDecimals(2380, 18))

    expect(await esNova.balanceOf(user0.address)).eq(expandDecimals(5, 18))
    expect(await esNova.balanceOf(vester.address)).eq(expandDecimals(2380, 18))
    expect(await feeNovaTracker.balanceOf(user0.address)).gt(expandDecimals(499, 18))
    expect(await feeNovaTracker.balanceOf(user0.address)).lt(expandDecimals(501, 18))
    expect(await feeNovaTracker.balanceOf(vester.address)).gt(expandDecimals(999, 18))
    expect(await feeNovaTracker.balanceOf(vester.address)).lt(expandDecimals(1001, 18))

    await rewardRouter.connect(user1).unstakeNova(expandDecimals(499, 18))
    ...
    expect(await feeNovaTracker.balanceOf(user0.address)).eq(expandDecimals(1500, 18))
    expect(await nova.balanceOf(user0.address)).gt(expandDecimals(201, 18)) // 2380 / 12 = ~198
    expect(await nova.balanceOf(user0.address)).lt(expandDecimals(203, 18))
    expect(await esNova.balanceOf(user0.address)).gt(expandDecimals(2182, 18)) // 5 + 2380 - 202  = 2183
    expect(await esNova.balanceOf(user0.address)).lt(expandDecimals(2183, 18))
  })
  ...
  it("handles existing pair tokens", async () => {
    ...
    expect(await esNova.balanceOf(user2.address)).gt(expandDecimals(2380, 18))
    expect(await esNova.balanceOf(user2.address)).lt(expandDecimals(2382, 18))
    expect(await esNova.balanceOf(user3.address)).gt(expandDecimals(1189, 18))
    expect(await esNova.balanceOf(user3.address)).lt(expandDecimals(1191, 18))
    ...
    expect(await feeNovaTracker.balanceOf(user0.address)).eq(expandDecimals(1000, 18))
    await esNova.mint(user0.address, expandDecimals(2380, 18))
    await vester.connect(user0).deposit(expandDecimals(2380, 18))

    expect(await feeNovaTracker.balanceOf(user0.address)).gt(0)
    expect(await feeNovaTracker.balanceOf(user0.address)).lt(expandDecimals(1, 18))
    ...
    const feeNovaTrackerBalance = await feeNovaTracker.balanceOf(user0.address)

    await esNova.mint(user0.address, expandDecimals(1190, 18))
    await vester.connect(user0).deposit(expandDecimals(1190, 18))

    expect(feeNovaTrackerBalance).eq(await feeNovaTracker.balanceOf(user0.address))

    await expect(rewardRouter.connect(user0).unstakeNova(expandDecimals(2, 18)))
      .to.be.revertedWith("RewardTracker: burn amount exceeds balance")

    await vester.connect(user0).withdraw()

    await rewardRouter.connect(user0).unstakeNova(expandDecimals(2, 18))
  })
})
