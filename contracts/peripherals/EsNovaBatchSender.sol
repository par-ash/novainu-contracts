...
contract EsNovaBatchSender {
    using SafeMath for uint256;

    address public admin;
    address public esNova;

    constructor(address _esNova) public {
        admin = msg.sender;
        esNova = _esNova;
    }

    modifier onlyAdmin() {
        require(msg.sender == admin, "EsNovaBatchSender: forbidden");
        _;
    }
    ...
    function send(
        ...
        for (uint256 i = 0; i < _accounts.length; i++) {
            IERC20(esNova).transferFrom(msg.sender, _accounts[i], _amounts[i]);
