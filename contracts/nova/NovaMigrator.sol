...
import "./interfaces/INovaIou.sol";
import "./interfaces/IAmmRouter.sol";
import "./interfaces/INovaMigrator.sol";

contract NovaMigrator is ReentrancyGuard, INovaMigrator {
    ...
    modifier onlyAdmin() {
        require(msg.sender == admin, "NovaMigrator: forbidden");
        _;
    }

    modifier onlySigner() {
        require(isSigner[msg.sender], "NovaMigrator: forbidden");
        _;
    }
    ...
    function initialize(
        ...
        require(!isInitialized, "NovaMigrator: already initialized");
        require(_whitelistedTokens.length == _iouTokens.length, "NovaMigrator: invalid _iouTokens.length");
        require(_whitelistedTokens.length == _prices.length, "NovaMigrator: invalid _prices.length");
        require(_whitelistedTokens.length == _caps.length, "NovaMigrator: invalid _caps.length");
        require(_lpTokens.length == _lpTokenAs.length, "NovaMigrator: invalid _lpTokenAs.length");
        require(_lpTokens.length == _lpTokenBs.length, "NovaMigrator: invalid _lpTokenBs.length");

        isInitialized = true;

        ammRouter = _ammRouter;
        novaPrice = _novaPrice;
    ...
    function migrate(
        ...
        require(isMigrationActive, "NovaMigrator: migration is no longer active");
        require(whitelistedTokens[_token], "NovaMigrator: token not whitelisted");
        require(_tokenAmount > 0, "NovaMigrator: invalid tokenAmount");

        if (hasMaxMigrationLimit) {
            migratedAmounts[msg.sender][_token] = migratedAmounts[msg.sender][_token].add(_tokenAmount);
            require(migratedAmounts[msg.sender][_token] <= maxMigrationAmounts[msg.sender][_token], "NovaMigrator: maxMigrationAmount exceeded");
        }

        uint256 tokenPrice = getTokenPrice(_token);
        uint256 mintAmount = _tokenAmount.mul(tokenPrice).div(novaPrice);
        require(mintAmount > 0, "NovaMigrator: invalid mintAmount");

        tokenAmounts[_token] = tokenAmounts[_token].add(_tokenAmount);
        require(tokenAmounts[_token] < caps[_token], "NovaMigrator: token cap exceeded");
        ...
        if (lpTokens[_token]) {
            ...
            require(tokenA != address(0), "NovaMigrator: invalid tokenA");
            require(tokenB != address(0), "NovaMigrator: invalid tokenB");
        ...
        INovaIou(iouToken).mint(msg.sender, mintAmount);
    }
    ...
    function signApprove(address _token, address _spender, uint256 _amount, uint256 _nonce) external nonReentrant onlySigner {
        ...
        require(!signedActions[msg.sender][action], "NovaMigrator: already signed");
    ...
    function getTokenPrice(address _token) public view returns (uint256) {
        ...
        require(price != 0, "NovaMigrator: invalid token price");
        return price;
    }

    function getIouToken(address _token) public view returns (address) {
        address iouToken = iouTokens[_token];
        require(iouToken != address(0), "NovaMigrator: invalid iou token");
        return iouToken;
    }
    ...
    function _validateAction(bytes32 _action) private view {
        require(pendingActions[_action], "NovaMigrator: action not signalled");
    }
    ...
    function _validateAuthorization(bytes32 _action) private view {
        ...
        if (count == 0) {
            revert("NovaMigrator: action not authorized");
        }
        require(count >= minAuthorizations, "NovaMigrator: insufficient authorization");
    }

    function _clearAction(bytes32 _action, uint256 _nonce) private {
        require(pendingActions[_action], "NovaMigrator: invalid _action");
